...
def create_link(src_file, target_path, is_soft_link=True):
    ...
    if os.path.lexists(target_path):
        # logger.debug(f"Target link already exists, skipping: {target_path}")
        return
...
def remove_link_and_empty_dirs(link_path):
    ...
    if os.path.islink(link_path) or os.path.isfile(link_path):
        try:
            os.remove(link_path)
            logger.debug(f"Removed link/file: {link_path}")
        except OSError as e:
            logger.error(f"Error removing link/file {link_path}: {e}")
            return
    ...
    while current_dir and current_dir != os.sep: # Stop at root or empty string
        try:
            ...
            # logger.debug(f"Error removing directory {current_dir}: {e}")
